                            ...
                            implements IAppErrorCodes
{
	...
	public Object visit (final AllItem item, final Object ctx)
    {
        ...
        try
        {
            ...
            final Integer [] columns = m_settings.getColumnOrder ();
            ...
            */
            ...
            {
            	Integer [] headerColumns = null;
                
                boolean odd = true;
                final ItemComparator order = m_typeSortComparators [PackageItem.getTypeMetadata ().getTypeID ()];
                for (Iterator<IItem> packages = item.getChildren (order); packages.hasNext (); odd = ! odd)
                {
	...
	public Object visit (final PackageItem item, final Object ctx)
    {
        ...
        try
        {
            ...
            final HTMLTable childSummaryTable = new HTMLTable ("100%", null, null, "0");
            {
                ...
                for (Iterator<IItem> srcORclsFiles = item.getChildren (order); srcORclsFiles.hasNext (); odd = ! odd)
                {
	...
	public Object visit (final SrcFileItem item, final Object ctx)
    {
        ...
        try
        {
            ...
            
            {
                ...
                int clsIndex = 0;                
                for (Iterator<IItem> classes = item.getChildren (order); classes.hasNext (); ++ clsIndex)
                {
...
//                    cell.setText ("class " + child.getName () + " methods:", true);
                    ...
                    for (Iterator<IItem> methods = cls.getChildren (order2); methods.hasNext (); odd = ! odd)
                    {
	...
	public Object visit (final ClassItem item, final Object ctx)
    {
        ...
        try
        {
            ...
            final HTMLTable childSummaryTable = new HTMLTable ("100%", null, null, "0");
            {
                ...
                for (Iterator<IItem> methods = item.getChildren (order); methods.hasNext (); odd = ! odd)
                {
                    final MethodItem method = (MethodItem) methods.next ();
    ...
    private Integer [] addHeaderRow (final IItem item, final HTMLTable table, final Integer [] columns)
    {
        if ($assert.ENABLED)
        {
        ...
        final Vector<Integer> headerColumns = new Vector<Integer> (columns.length);
        ...
        return headerColumns.toArray(new Integer[0]);
    }
    
    /*
     * No header row, just data rows.
     */
    private void addItemRow (final IItem item, final boolean odd, final HTMLTable table, final Integer [] columns, final String nameHREF, final boolean anchor)
    {
        if ($assert.ENABLED)
        {
            $assert.ASSERT (item != null, "null input: item");
    ...
    private void addClassRow (final ClassItem item, final int clsIndex, final HTMLTable table, final Integer [] columns,
    ...
    private void addClassItemRow (final IItem item, final boolean odd, final HTMLTable table, final Integer [] columns, final String nameHREF, final boolean anchor)
    {
        if ($assert.ENABLED)
        {
            $assert.ASSERT (item != null, "null input: item");
...
//    }
    ...
    {
        final LinkedList<IItem> _result = new LinkedList<IItem> ();
        ...
        for (Iterator<IItem> i = _result.iterator (); i.hasNext (); -- j)
        {
