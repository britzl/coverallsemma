...
final class ConstantCollection implements IConstantCollection
{
	...
	public Object clone ()
    {
        try
        {
            ...
            _clone.m_constants = new ArrayList<CONSTANT_info> (constants_count);
            for (int c = 0; c < constants_count; ++ c)
            {
                final CONSTANT_info constant = (CONSTANT_info) m_constants.get (c);
                _clone.m_constants.add (constant == null ? null : (CONSTANT_info)constant.clone ());
            }
    ...
    {
        m_constants = capacity < 0 ? new ArrayList<CONSTANT_info> () : new ArrayList<CONSTANT_info> (capacity);
    }
